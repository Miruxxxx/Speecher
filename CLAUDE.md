...
- **The session header names the engine that actually ran, not the configured one.** `main.py` builds `session_meta` before the model has tried to load, so after a fallback it would name an engine that produced nothing — every export and every journal file carried that lie. The loader calls `overlay.set_engine(actual, model, requested=…)` once the engine is up, which corrects the in-memory meta and appends a fresh `{"t":"meta"}` record; `session_io.read_session` takes the **last** meta it sees, so no new record type and no `JOURNAL_VERSION` bump. Anything else learned late about the session belongs in `update_meta` for the same reason.
...
- **No module-level import of `torch`, `transformers`, `faster_whisper`, `librosa`, `accelerate`, `soxr` or `keyring` anywhere in `src/`** — every one of them is imported inside the function that needs it. That is what keeps the window instant *and* what makes CI cost 30 s on four packages (`requirements-test.txt`) instead of ten minutes on 3 GB of wheels. If CI ever fails with an `ImportError`, a module-level import has appeared; that is the signal, not a reason to add torch to the CI requirements.
